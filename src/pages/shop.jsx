...
const ShopPage = () => {
  ...
  useEffect(() => {
    if (!isLoading && !isError && products?.data?.length > 0 && !priceInitializedRef.current) {
      ...
      const urlMaxPrice = query.maxprice ? parseFloat(query.maxprice) : maxPrice;
      
      // Validate URL prices
      const validMinPrice = isNaN(urlMinPrice) || urlMinPrice < 0 ? 0 : Math.min(urlMinPrice, maxPrice);
      const validMaxPrice = isNaN(urlMaxPrice) || urlMaxPrice <= 0 ? maxPrice : Math.min(urlMaxPrice, maxPrice);
  ...
  useEffect(() => {
    ...
    const minPriceChanged = Math.abs(currentMinPrice - debouncedPriceValue[0]) > 0.01;
    const maxPriceChanged = Math.abs(currentMaxPrice - debouncedPriceValue[1]) > 0.01;
  ...
  const productItemsFiltered = useMemo(() => {
    ...
    const minPrice = debouncedPriceValue[0] > 0 ? debouncedPriceValue[0] : (query.minprice ? parseFloat(query.minprice) : 0);
    const maxPrice = debouncedPriceValue[1] > 0 ? debouncedPriceValue[1] : (query.maxprice ? parseFloat(query.maxprice) : 0);
    if (maxPrice > 0) {
    ...
    if (query.category) {
      ...
      list = list.filter((p) => {
        if (p.category?.slug && (p.category.slug === decodedCategory || p.category.slug === query.category)) return true;
        if (p.parent) {
          const parentSlug = p.parent.toLowerCase().replace(/'/g, '').replace("&", "").split(" ").join("-");
          if (parentSlug === decodedCategory || parentSlug === query.category) return true;
        }
        if (p.category?.name) {
          const nameSlug = p.category.name.toLowerCase().replace(/'/g, '').replace("&", "").split(" ").join("-");
          if (nameSlug === decodedCategory || nameSlug === query.category) return true;
        }
        return false;
      });
    }
    if (query.subCategory) {
      list = list.filter((p) => p.children && p.children.toLowerCase().replace("&", "").split(" ").join("-") === query.subCategory);
    }
    if (query.color) {
      list = list.filter((product) => {
        ...
        for (let i = 0; i < product.imageURLs.length; i++) {
          ...
          if (color && color?.name?.toLowerCase().replace("&", "").split(" ").join("-") === query.color) return true;
        }
        return false;
      });
    }
    if (query.brand) {
      list = list.filter((p) => p.brand?.name && p.brand.name.toLowerCase().replace("&", "").split(" ").join("-") === query.brand);
    }
    return list;
  }, [
    products?.data,
